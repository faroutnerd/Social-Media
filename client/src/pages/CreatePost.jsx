...
const CreatePost = () => {
  ...
  const handleSubmit = async () => {
    if(!images.length && !content){
     return toast.error('Please select at least one image or text');
    }
    setLoading(true);
            ...
            <div className='flex flex-wrap gap-2 mt-4'>
              {
                images.map((image, i)=>(
                  <div key={i} className='relative group'>
                    ...
                    <div onClick={()=>setImages(images.filter((_, index) => index !== i))} className='absolute  inset-0 hidden group-hover:flex justify-center items-center top-0 right-0 left-0 bg-black/40 rounded-md cursor-pointer'>
